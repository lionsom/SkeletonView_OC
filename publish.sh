...
#
# Version:3.1
# Swift版本：移除--use-libraries
# 解决git commit 无数据时候运行失败问题
#
...
localVerifyLib(){
    ...
    if ! pod lib lint --skip-import-validation --allow-warnings --sources="${source1},${source2}"; then echo -e "${RED}验证失败${NC}🌧🌧🌧"; exit 1; fi
...
#    if ! pod spec lint --skip-import-validation --allow-warnings --sources="${source1},${source2}"; then echo -e "${RED}验证失败${NC}🌧🌧🌧"; exit 1; fi
#    echo -e "${GREEN}验证成功${NC}🚀🚀🚀"
}
...
publishLib(){
    echo -e "${GREEN}\n第七步：准备发布${tag}版本${NC}⏰⏰⏰"
    if ! pod repo push PaasSpecs ${SpecName} --allow-warnings --sources="${source1},${source2}" --allow-warnings --skip-import-validation; then echo -e "${RED}发布${tag}版本失败${NC}🌧🌧🌧"; exit 1; fi
